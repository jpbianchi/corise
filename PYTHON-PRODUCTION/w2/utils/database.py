...
class DB:
    ...
    def insert(self, process_id, start_time, file_name=None, file_path=None,
        ...
        """
    ######################################## YOUR CODE HERE ##################################################
        argsnames = ','.join(['process_id', 'file_name', 'file_path', 'description', 'start_time', 'end_time', 'percentage'])
        args = [process_id, file_name, file_path, description, start_time, end_time, percentage]
        self._connection.execute(f'''
                    INSERT INTO {self._table_name} ({argsnames})
                    VALUES ({','.join(['?'] * len(args))})''', args)
        self._connection.commit()
    ######################################## YOUR CODE HERE ##################################################
