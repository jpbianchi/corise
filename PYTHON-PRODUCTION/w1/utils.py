...
class DataReader:
    ...
    ######################################## YOUR CODE HERE ##################################################
        with open(self._fp, 'r') as f:
            for line in f:
                line = line.strip()
                if line:  # let's skip empty lines
                    line = line.split(self._sep)
                    yield dict(zip(self._col_names, line))
        # because we defined it in __iter__, which is called when we iterate on stgh
        # whether it's an iterator (like a list) or a generator (which is also an iterator)
        # so we cannot do next() on data_reader directly since it wont' call __iter__
        # we can use it in a for loop because it will call __iter__ automatically
        # or we create the generator with data_reader.__iter__() or a comprehension
